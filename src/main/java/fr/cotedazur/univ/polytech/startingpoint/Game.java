...
public class Game {
    ...
    public Boolean play(Player player){
        ...
        LOGGER.log(Level.FINE,"Le dé météo tombe sur "+weather);
        actOnWeather(weather, player);

        LOGGER.log(Level.FINE,player.getName()+" choisit les actions : "+playerActions[0]+" & "+playerActions[1]);
    ...
    public Boolean choiceObjective(Player player){
        ...
        LOGGER.log(Level.FINE,player.getName()+" ne peut plus choisir d'objectif");
        LOGGER.log(Level.FINE,player.getName()+" a 5 objectif non validé et est prié de les validé");
        LOGGER.log(Level.FINE, player.getUnMetObjectives().toString());
        return false;
    }
    ...
    public Boolean choicePlot(Player player){
        if (deckOfPlots.size()!=0 ) {
            ...
            LOGGER.log(Level.FINE,player.getName()+" a ajouté la parcelle suivante :"+board.getLastHexPlot());
            LOGGER.log(Level.FINE,"la liste des parcelles dans le jeu aprés le choix:"+board);
            return true;
    ...
    public Boolean choiceAnIrrigation(Player p){
        ...
        LOGGER.log(Level.FINE,"Il y a plus de canal d'irrigation disponible");
        return false;
    ...
    public Boolean placeAnIrrigation(Player p){
        ...
        if(irrigationStock.add(canal.get(),src.get(),dst.get(),board)){
            LOGGER.log(Level.FINE, String.valueOf(canal.get()));
        }else{
    ...
    public boolean movePanda(Player player){
        LOGGER.log(Level.FINE,"la position du panda avant deplacement "+panda.getPosition());
        LOGGER.log(Level.FINE,"la liste des parcelles dans le jeu :"+board);
        Random rand = new Random();
        ...
        if(movePossibilities.size()!=0){
            ...
            LOGGER.log(Level.FINE,"la position du panda aprés deplacement"+panda.getPosition());
            eatIfBamboo(next, player);
            return true;
        }
        LOGGER.log(Level.FINE,"Impossible de faire deplacer le panda");
        return false;
    }
 
    public boolean moveGardener(Player player){
        LOGGER.log(Level.FINE,"La position du jardinier avant deplacement "+gardener.getPosition());
        Random rand = new Random();
        List<HexPlot> movePossibilities = board.getNewPositionPossibilities();
        if(movePossibilities.size()!=0){
            int randNumber = rand.nextInt(movePossibilities.size());
            HexPlot next = movePossibilities.get(randNumber);
            gardener.move(next);
            LOGGER.log(Level.FINE,"La position du jardinier aprés deplacement "+gardener.getPosition());
            next.addBamboo();
            next.plotNeighbor()
                    .stream()
        ...
        LOGGER.log(Level.FINE,"Impossible de déplacer le jardinier");
        return false;
    }

    public boolean eatIfBamboo(HexPlot plot, Player player){
        if(plot.getBamboos().size()!=0){
            LOGGER.log(Level.FINE,"il y a de bambou sur cette parcelle");

            if (plot.getImprovement()==FENCE)
                LOGGER.log(Level.FINE,"cette parcelle est protégée par un enclos");
            else {
                LOGGER.log(Level.FINE,"panda mange un bambou de couleur " + plot.getColor());
                player.eatenBamboos.add(plot.getBamboos().get(0));
                plot.getBamboos().remove(0);
            }
            return true;
        }else{
            LOGGER.log(Level.FINE,"il y a pas de bambou sur cette parcelle");
            return false;
        }
    }
    ...
    public boolean placeImprovement(Player player){
        ...
        LOGGER.log(Level.FINE,"La parcelle " + plotForImrovement + " a été amélioré par " + improvement);
        return true;
    }
    ...
    public void actOnWeather(Dice.Condition weatherCondition, Player player){
        switch (weatherCondition) {
            ...
            case SUN:
                ...
                LOGGER.log(Level.FINE,player.getName()+" choisit une action supplémentaire : "+actionsToPickFrom.get(pick));
            ...
            case STORM:
                ...
                panda.pandaMove(next);
                LOGGER.log(Level.FINE,player.getName()+" déplace le panda à : "+panda.getPosition());
                ...
                if (! hasEaten) {
                    ...
                    LOGGER.log(Level.FINE,"panda mange un bambou de couleur " + next.getColor());
                }
                break;
    ...
    public void display(){
        for (Player p:playerList) {
         LOGGER.log(Level.FINE, String.valueOf(p));
        }
    }
}
